...
use App\Jobs\UpdatePurchaseExpirationInfoJob;

class ExpirationWorker extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'ExpirationWorker:run';
     ...
     * @var string
     */
    protected $description = 'Worker for updating the subscriptions states based of the expiration date and the ios/google verification.';
    ...
    public function handle()
    {
        $purchases = DB::select(
            ...
            WHERE pr.`status` = 1 AND pr.`state` <> 2 AND pr.`expire_date` IS NOT NULL AND pr.`expire_date` <= ?;',
        ...
        foreach ($chunks as $k => $purchasesChunk) {
            ...
            $this->info('UpdatePurchaseExpirationInfoJob::dispatch');
            Queue::push(new UpdatePurchaseExpirationInfoJob($purchasesChunk));
        }
